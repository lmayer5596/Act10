...
#This code chunk loads all necessary packages on the date in which the RMD project was started
...
#This code chunk initializes the collatz function which calculates the stopping time for a given number
...
#This code chunk allows the collatz function to receive a vector as input and will output a vector as well. This enables us to calculate the stopping time for many numbers at one time.
...
#This code chunk creates a histogram of the stopping times calculated for numbers 1 to 10000
ggplot(
...
#This code chunk creates a scatterplot where we look at the price vs weight of each diamond, with a grouping by color. 
ggplot(diamonds) + #creates framework
aes(x = carat, y = price, colour = color) + #saves carat on horizontal, price on vertical, and color as color
geom_point(shape = "circle", size = 1.25) + #initializes the glyphs as points
scale_color_manual( #creates brighter colors for each color
values = c(D = "#FF002B",
E = "#FFB300",
F = "#FFFC00",
G = "#1FFF09",
...
expand_limits(y = seq(0,20000)) #expands the vertical axis to contain all points
```
...
#This code chunk creates a summary table which shows summary statistics for each cut of diamond in the dataset. 
...
First, I have included an image of my working directory. All data for this assignment is from packages in R, so I need no other data files in the directory. The directory includes this RMD file, the project for this assignment, and the three knitted outputs. 
...
#This code chunk combines all previous code chunks into a vector then displays them all together without evaluating the code.
```
